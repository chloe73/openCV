...
int main(int argc, const char * argv[]) {
    ...
    threshold(img, img, 180, 255, THRESH_OTSU);
    imshow("binar", img);
    ...
    for(int i = 0; i < cnt; i++){
        ...
        if (area < 70) continue;
        else if (area > 1000) continue;
        else if (width > 700) continue;
        else if (height > 700) continue;
            
        rectangle(origin, Rect(x-2, y-2, width+4, height+4), Scalar(0, 255, 255));
        count++;
        stringstream str;
        str << count;
        putText(origin, str.str(), Point(x-2, y-2), FONT_HERSHEY_PLAIN, 1.2, Scalar(255, 255, 255));
    }
